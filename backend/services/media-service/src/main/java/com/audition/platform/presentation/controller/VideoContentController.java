...
public class VideoContentController {
    ...
    @GetMapping("/{id}")
    @Operation(summary = "비디오 상세 조회", description = "MVP_03: 공개 범위 및 삭제 상태 검증")
    public ResponseEntity<VideoContentDto> getVideo(
            @PathVariable Long id,
            @RequestHeader(value = "Authorization", required = false) String authHeader
    ) {
        Long requesterId = securityUtils.getUserIdFromAuthHeader(authHeader);
        VideoContentDto video = videoContentService.getVideo(id, requesterId);
    ...
    @PostMapping("/upload")
    @Operation(summary = "비디오 파일 업로드", description = "작업: 2026_12_personal_channel_media")
    public ResponseEntity<VideoContentDto> uploadVideoFile(
            ...
            @RequestParam(required = false) String visibility // PUBLIC, PRIVATE, FOLLOWERS_ONLY
    ) {
        Long userId = securityUtils.getUserIdFromAuthHeaderOrThrow(authHeader);
        VideoContentDto created = videoContentService.uploadVideoFile(
                userId, file, title, description, category, videoType, visibility);
        return ResponseEntity.status(HttpStatus.CREATED).body(created);
    }
    ...
    @GetMapping("/my")
    @Operation(summary = "내 비디오 목록", description = "현재 로그인한 사용자의 모든 비디오 목록 (공개 범위 무관)")
    public ResponseEntity<Page<VideoContentDto>> getMyVideos(
        ...
        Long userId = securityUtils.getUserIdFromAuthHeaderOrThrow(authHeader);
        Page<VideoContentDto> videos = videoContentService.getMyVideos(userId, pageable);
        return ResponseEntity.ok(videos);
    }

    @GetMapping("/channel/{userId}")
    @Operation(summary = "개인 채널 비디오 목록", description = "특정 사용자의 공개 비디오 목록")
    public ResponseEntity<Page<VideoContentDto>> getChannelVideos(
