...
import com.audition.platform.application.service.AgencyStatsService;
...
public class AuditionController {
    ...
    @Operation(summary = "오디션 상태 전이", description = "MVP_01: DRAFT → OPEN → CLOSED → FINALIZED (AGENCY만 가능)")
    public ResponseEntity<AuditionDto> updateAuditionStatus(
            @PathVariable Long id,
            @RequestBody @Valid UpdateAuditionStatusRequest request
    ) {
        AuditionDto updated = auditionService.updateAuditionStatus(id, request);
        return ResponseEntity.ok(updated);
    }
    ...
    @Operation(summary = "기획사 대시보드 통계", description = "현재 로그인한 기획사의 통계 조회")
    public ResponseEntity<AgencyDashboardStatsDto> getDashboardStats() {
        Long businessId = currentUserContext.getCurrentUserIdOrThrow();
        AgencyDashboardStatsDto stats = agencyStatsService.getDashboardStats(businessId);
        return ResponseEntity.ok(stats);
    }

    @GetMapping("/{id}/stats")
    @Operation(summary = "오디션별 통계", description = "오디션별 단계별 지원자 수 통계")
    public ResponseEntity<AuditionStatsDto> getAuditionStats(@PathVariable Long id) {
        AuditionStatsDto stats = agencyStatsService.getAuditionStats(id);
        return ResponseEntity.ok(stats);
    }
}
