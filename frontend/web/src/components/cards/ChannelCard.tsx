...
export default function ChannelCard({ channel }: ChannelCardProps) {
  ...
  const displayName = name || tChannel('nameFallback')
  ...
  return (
    ...
    >
      <div style={{ margin: '0 auto', width: CHANNEL_CARD.profileSizePx, height: CHANNEL_CARD.profileSizePx, borderRadius: '50%', overflow: 'hidden', position: 'relative' }}>
        {avatar ? (
          <Image src={avatar} alt={displayName} fill style={{ objectFit: 'cover' }} unoptimized />
        ) : (
      ...
      >
        {description || tChannel('descFallback')}
      </p>
      ...
      >
        {tChannel('subscribersVideos', { subs: subscribers.toLocaleString(), videos: videoCount.toLocaleString() })}
      </div>
