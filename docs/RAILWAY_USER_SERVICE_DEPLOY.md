...
5. **서비스 설정**
  ...
  - Root Directory: `backend/services/user-service` ⚠️ **중요!**
...
- [ ] GitHub 저장소 연결 완료
- [ ] Root Directory: `backend/services/user-service` 설정
...
1. **Audition Service 배포** (동일한 방법, Root Directory: `backend/services/audition-service`)
2. **Media Service 배포** (Root Directory: `backend/services/media-service`)
3. **Gateway 배포** (다른 서비스 URL 필요, Root Directory: `backend/services/gateway`)
